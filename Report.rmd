...
analysis <- CategoriesAndAgeGroups%>%
  filter(Cause == "B00")%>%
  pivot_longer(longer, names_to="Age", values_to="DeathCause")%>%
  filter(Age=="80-84")

ggplot(data=analysis, aes(y=OverallDeaths, x=DeathCause))+
  geom_point()+
  geom_smooth()

fit <- lm(DeathCause~OverallDeaths,data=analysis)
summary(fit)
plot(fit)

```
The overall deaths per 100.000 Inhabitants get compared to one deathcause. It shows that its possible to compare these variables this way.
```{r}
analysis2 <- analysis%>%
...
plot(fit2)
```
Compare East and West Germany with the unified Germany. In order to see if east and west are near together.
Calculate the probability of the outcome. If east or west germany is more close.
...
11. All functions into one code Block in the beginning
